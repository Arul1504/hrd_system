<?php
/**
 * File: monitoring_kontrak.php (FINAL HRD)
 * Sama dengan versi admin, tapi role HRD & dashboard HRD
 */
require '../config.php';
...
$stmt->bind_param("i",$id_karyawan_hrd);
$stmt->execute();
$res = $stmt->get_result();
$hrd_info = $res->fetch_assoc();
$stmt->close();

$sql_pending_requests = "SELECT COUNT(*) AS total_pending FROM pengajuan WHERE status_pengajuan = 'Menunggu'";
$r = $conn->query($sql_pending_requests);
$total_pending = $r->fetch_assoc()['total_pending']??0;
...
";
$params[]=$due_limit; $params[]=$due_limit; $types.='ss';
...
<div class="container">
  ...
  <aside class="sidebar">
    ...
    <nav class="sidebar-nav">
      <ul>
        ...
        <li class="dropdown-trigger">
          <a href="#" class="dropdown-link"><i class="fas fa-file-alt"></i> Data Pengajuan <span class="badge"><?= $total_pending ?></span><i class="fas fa-caret-down"></i></a>
          <ul class="dropdown-menu">
  ...
  <main class="main-content">
    <div class="main-header">
      ...
      <span class="current-date">Menampilkan kontrak s.d <b><?= e(dmy($due_limit)) ?></b></span>
    </div>
    ...
    <div class="toolbar">
      <form class="search-form" method="get">
        <div class="search-filter-container">
          ...
          <div class="filter-box">
            <select name="status">
              <?php foreach(['AKTIF','TIDAK AKTIF',''] as $s): $label=$s?:'Semua Status'; $sel=$filter_status===$s?'selected':''; ?>
              <option value="<?= e($s) ?>" <?= $sel ?>><?= e($label) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="filter-box">
            <input type="number" name="due_in_days" min="1" value="<?= e($due_in_days) ?>">
          </div>
          <div class="search-box">
            <input type="text" name="s" placeholder="Cari nama/NIK" value="<?= e($search) ?>">
            <button type="submit"><i class="fas fa-search"></i></button>
          </div>
          <button class="add-button" type="submit"><i class="fas fa-filter"></i> Terapkan</button>
        </div>
      </form>
    </div>
    ...
    <div class="data-table-container">
      <table>
        <thead>
          <tr><th>Nama</th><th>Proyek</th><th>Jabatan</th><th>Mulai</th><th>Jatuh Tempo</th><th>Status</th><th>Aksi</th></tr>
        </thead>
        <tbody>
        <?php if(!$rows): ?>
        ...
        ?>
          <tr>
            ...
            <td><?= e(dmy($due)) ?> <?php if($daysLeft!==null): ?><span class="<?= $badgeClass ?>"><?= e($daysLeft) ?>h</span><?php endif; ?></td>
            <td><span class="badge"><?= e($r['status']) ?></span></td>
            <td>
              <div class="action-dropdown">
                <button class="action-menu-trigger" onclick="toggleMenu(this)">Aksi <i class="fas fa-caret-down"></i></button>
                <ul class="action-menu">
                  <li><a href="#" onclick="event.preventDefault(); openPerpanjang(<?= (int)$r['id_karyawan'] ?>,'<?= e($r['nama_karyawan']) ?>','<?= e($r['proyek']) ?>')"><i class="fas fa-calendar-plus"></i> Perpanjang Kontrak</a></li>
                  <li><a href="#" onclick="event.preventDefault(); openSurat(<?= (int)$r['id_karyawan'] ?>,'<?= e($r['nama_karyawan']) ?>','<?= e($r['proyek']) ?>','<?= e($r['jabatan']) ?>')"><i class="fas fa-file-signature"></i> Buat Surat Tugas</a></li>
                  <li><a href="#" onclick="event.preventDefault(); openUpload(<?= (int)$r['id_karyawan'] ?>,'<?= e($r['nama_karyawan']) ?>','<?= e($r['proyek']) ?>')"><i class="fas fa-upload"></i> Upload Surat Tugas</a></li>
                </ul>
              </div>
            </td>
          </tr>
