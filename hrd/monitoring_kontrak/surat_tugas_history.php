...
$nik_user_hrd = $hrd_info['nik_ktp'] ?? 'Tidak Ditemukan';
$jabatan_user_hrd = $hrd_info['jabatan'] ?? 'Tidak Ditemukan';
$stmt_hrd_info->close();
...
<head>
    ...
    <title>Riwayat Surat Tugas (HRD)</title>
    <link rel="stylesheet" href="../style.css">
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css">
    <style>
        .table-container { padding:16px; background:#fff; border-radius:10px; box-shadow:0 1px 4px rgba(0,0,0,0.06);}
        .table { width:100%; border-collapse:collapse;}
        .table th,.table td { border-bottom:1px solid #eef0f3; padding:12px; text-align:left;}
        .table th { background:#f0f2f5; font-weight:600;}
        .table tr:hover { background:#f9fafb;}
        .btn-action { background:#2563eb; color:#fff; border:none; padding:8px 12px; border-radius:8px;
        ...
        .filter-toolbar select, .filter-toolbar button { padding:8px 12px; border:1px solid #ddd; border-radius:8px;}
        .filter-toolbar button { background:#2ecc71; color:white; cursor:pointer;}
        ...
        <div class="table-container">
            <table class="table">
                ...
                <?php if (empty($surat_history)): ?>
                    <tr><td colspan="6" style="text-align:center">Tidak ada riwayat surat tugas.</td></tr>
                <?php else: foreach ($surat_history as $row): ?>
                    <tr>
                        ...
                        <td>
                            <a href="surat_tugas_view.php?id=<?= e($row['id']) ?>" target="_blank" class="btn-action" title="Lihat Surat Tugas">
