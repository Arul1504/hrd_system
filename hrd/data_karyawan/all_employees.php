...
// (Lengkap dengan filter dinamis Status Karyawan, modal add/view/edit, dan validasi dasar)
// ==========================

require '../config.php'; // pastikan ada fungsi helper e($v) di sini

if (session_status() === PHP_SESSION_NONE) {
    session_start();
}

// Flash utk modal Add (opsional, jika process_add memakai session flash)
...
if (isset($_GET['status'])) {
    ...
    if (isset($map[$_GET['status']])) {
        $cls = in_array($_GET['status'], ['absen_failed', 'nik_duplicate', 'error']) ? 'error' : 'success';
...
if ($filter_status !== '') {
    // user memang minta filter status tertentu (AKTIF / TIDAK AKTIF)
    $sql .= " AND UPPER(status) = UPPER(?)";
    $params[] = &$filter_status;
    $types .= "s";
} else {
    // DEFAULT: jangan tampilkan karyawan tidak aktif
...
// Options filter (untuk kondisi proyek kosong)
...
<body>
    <div class="container">
        ...
        <main class="main-content">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="#" class="action-btn view-btn" data-id="<?= e($employee['id_karyawan']) ?>"
                                            title="Lihat"
                                            onclick="openViewModal('<?= e($employee['id_karyawan']) ?>'); return false;">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="#" class="action-btn edit-btn" data-id="<?= e($employee['id_karyawan']) ?>"
                                            data-proyek="<?= e($employee['proyek'] ?? '') ?>" title="Ubah"
                                            onclick="openEditModal('<?= e($employee['id_karyawan']) ?>','<?= e($employee['proyek'] ?? '') ?>'); return false;">
            ...
            <!-- Modal View (DETAIL) – ringkas (AJAX get_employee_details.php) -->
            ...
            <!-- Modal Edit (UBAH DATA) – field dinamis sesuai proyek -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            window.closeModal = function () { if (addModal) addModal.style.display = "none"; };
            ...
            document.addEventListener('keydown', function (e) {
                if (e.key === 'Escape') { closeModal(); closeViewModal(); closeEditModal(); }
            });
            ...
            function inputFor(label) {
                ...
                if (slug === "role") {
                    return `<label>${label}<select name="${name}" ${req}><option value="">— Pilih —</option><option value="KARYAWAN">KARYAWAN</option><option value="HRD">HRD</option></select></label>`;
                }
