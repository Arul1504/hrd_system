...
$q = $conn->prepare("SELECT st.*, k.nama_karyawan, k.nik_ktp, k.jabatan, k.proyek, k.join_date, k.sales_code, k.alamat_email,
...
<body>
      ...
      <nav class="sidebar-nav">
        <ul>
          ...
          <li><a href="../monitoring_kontrak/monitoring_kontrak.php"><i class="fas fa-calendar-alt"></i> Monitoring
              Kontrak</a></li>
          <li class="active"><a href="surat_tugas_history.php"><i class="fas fa-file-alt"></i> Riwayat Surat Tugas</a>
          </li>
      ...
      <section class="surat" id="surat-tugas-dokumen">
        ...
        <table class="tbl">
          ...
          <tr>
            ...
            <td class="colon">:</td>
            <td><?= nl2br(e($alamatKantor)) ?></td>
          </tr>
          <tr>
            <td class="lbl">Efektif Penempatan di Cabang</td>
            <td class="colon">:</td>
            <td><?= e(idDate($efektif)) ?></td>
          </tr>
        </table>
  ...
  <script>
    function sendSuratAsEmail() {
      ...
      const emailTujuan = "<?= e($r['alamat_email'] ?? '') ?>"; // Gunakan kolom 'alamat_email' yang ada di query
      ...
      const opt = {
        margin: [5, 5, 5, 5],
        filename: `Surat-Tugas-${fileNamePrefix}.pdf`, // PERBAIKAN 1: Gunakan backticks
      ...
      worker.output('blob').then(function (pdfBlob) {
        ...
        formData.append('surat_pdf', pdfBlob, `Surat-Tugas-${fileNamePrefix}.pdf`);
        formData.append('id_surat', idSurat);
        formData.append('email_tujuan', emailTujuan);

        // Pastikan 'send_surat_email.php' berada di folder yang sama (surat_tugas/)
    ...
    function downloadSuratAsPDF(fileNamePrefix) {
      ...
      const opt = {
        margin: [6, 6, 6, 6],
        // PERBAIKAN 2: Gunakan backticks untuk template literal
