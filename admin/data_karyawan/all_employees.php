...
<head>
    ...
    <style>
        ...
        .absen-btn{background:#2ecc71;color:#fff}.absen-btn:hover{background:#27ae60}
        .action-buttons{display:flex;gap:10px;align-items:center}
        .download-btn{padding:8px 14px;border-radius:6px;background:#3498db;color:#fff;text-decoration:none;font-size:14px;display:flex;align-items:center;gap:6px;transition:background .2s}
        .download-btn:hover{background:#2980b9}
        .download-btn i{font-size:16px}
        .download-btn:nth-child(1){background:#e74c3c}.download-btn:nth-child(1):hover{background:#c0392b}
        .download-btn:nth-child(2){background:#27ae60}.download-btn:nth-child(2):hover{background:#1e8449}
        ...
        #addEmployeeModal,#viewEmployeeModal,#editEmployeeModal{z-index:9999}
        .tag{display:inline-block;padding:3px 8px;border-radius:999px;border:1px solid #ddd;color:#333;font-size:12px}
        .muted{color:#777;font-size:12px;margin-top:4px;display:block}
        .action-btn{display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;border:none;border-radius:6px;cursor:pointer;position:relative;z-index:30}
        .view-btn{background:#17a2b8;color:#fff}.edit-btn{background:#f1c40f;color:#fff}.delete-btn{background:#e74c3c;color:#fff}
        .action-btn i{pointer-events:none}
        .main-content{position:relative;z-index:20}
        .modal .modal-content{max-width:980px;border-radius:16px;overflow:hidden}
        .modal-head{display:flex;align-items:center;gap:14px;background:#f7f9fc;padding:16px 20px;border-bottom:1px solid #eef0f3}
        ...
        .emp-avatar{width:64px;height:64px;border-radius:50%;background:#eef2f7;color:#4a5568;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:18px;border:1px solid #e8edf3}
        .emp-meta{display:flex;flex-direction:column;gap:4px}
        .emp-name{font-weight:700;font-size:1.05rem;margin:0}
        .emp-sub{color:#6b7280;font-size:.9rem;margin:0}
        .badges{display:flex;gap:8px;flex-wrap:wrap;margin-top:6px}
        .badge{font-size:.75rem;padding:4px 10px;border-radius:999px;border:1px solid #e6eaf0;background:#f9fafb}
        .badge.green{border-color:#d1fae5;background:#ecfdf5;color:#047857}
        .badge.amber{border-color:#fef3c7;background:#fffbeb;color:#92400e}
        .tabs{display:flex;gap:6px;border-bottom:1px solid #eef0f3;margin:12px 0}
        .tab-btn{border:none;background:transparent;padding:10px 12px;border-radius:10px;cursor:pointer;font-weight:600;color:#6b7280}
        .tab-btn.active{background:#eef2ff;color:#1d4ed8}
        .tab-panel{display:none;padding-top:6px}.tab-panel.active{display:block}
        .grid-2{display:grid;gap:12px;grid-template-columns:1fr 1fr}
        .grid-3{display:grid;gap:12px;grid-template-columns:1fr 1fr 1fr}
        @media(max-width:900px){.grid-3{grid-template-columns:1fr 1fr}}
        @media(max-width:720px){.grid-2,.grid-3{grid-template-columns:1fr}}
        .detail-card{border:1px solid #eef0f3;border-radius:12px;padding:14px 14px 10px;background:#fff}
        .detail-label{font-size:.75rem;color:#6b7280;margin:0 0 6px}
        .detail-value{font-weight:600;color:#111827;margin:0;word-break:break-word}
        .modal-foot{display:flex;justify-content:space-between;align-items:center;padding:14px 20px;border-top:1px solid #eef0f3;background:#fafbfc}
        .primary{background:#2563eb;color:#fff;border:none;padding:10px 14px;border-radius:10px;cursor:pointer}
        .primary:hover{background:#1d4ed8}
        .ghost{background:transparent;border:1px solid #e5e7eb;padding:10px 14px;border-radius:10px;cursor:pointer}
        #editEmployeeModal .modal-content{max-width:980px}
        #edit_dynamic_fields .form-group label{display:block;font-weight:600;margin-bottom:6px}
        #edit_dynamic_fields input,#edit_dynamic_fields select,#edit_dynamic_fields textarea,
        #dynamic_fields input,#dynamic_fields select,#dynamic_fields textarea{width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:10px;font:inherit}
        .badge.red{background:#ef4444;color:#fff;padding:2px 8px;border-radius:999px;font-size:12px;}
    </style>
</head>
<body>
<div class="container">
    ...
    <main class="main-content">
        ...
        <div class="toolbar">
            <div class="search-filter-container">
                <form action="all_employees.php" method="GET" class="search-form">
                    <div class="search-box">
                        <input type="text" id="searchInput" name="search" placeholder="Cari NIK atau Nama..." value="<?= e($search_query) ?>" />
            ...
            <div class="action-buttons">
                ...
                <button class="add-button" onclick="openModal()"><i class="fas fa-plus-circle"></i> Tambah Karyawan</button>
            </div>
        </div>
        ...
        <div class="data-table-container">
            <table id="employeeTable">
                ...
                <?php if (empty($employees)): ?>
                    <tr><td colspan="7" style="text-align:center;">Tidak ada data karyawan yang ditemukan.</td></tr>
                <?php else:
                            ...
                            <td>
                                <div class="action-btn-group">
                                    <a href="#" class="action-btn view-btn" data-id="<?= e($employee['id_karyawan']) ?>"
                                       title="Lihat" onclick="openViewModal('<?= e($employee['id_karyawan']) ?>'); return false;"><i class="fas fa-eye"></i></a>
                                    <a href="#" class="action-btn edit-btn" data-id="<?= e($employee['id_karyawan']) ?>"
                                       data-proyek="<?= e($employee['proyek'] ?? '') ?>" title="Ubah"
                                       onclick="openEditModal('<?= e($employee['id_karyawan']) ?>','<?= e($employee['proyek'] ?? '') ?>'); return false;"><i class="fas fa-edit"></i></a>
                                    <button type="button" class="action-btn delete-btn"
                                            data-id="<?= e($employee['id_karyawan']) ?>" title="Hapus"
                                            onclick="confirmDelete('<?= e($employee['id_karyawan']) ?>')"><i class="fas fa-trash-alt"></i></button>
                                </div>
                            </td>
                        </tr>
        ...
        <div id="addEmployeeModal" class="modal" style="display:none;">
            <div class="modal-content">
                ...
                <?php endif; ?>
                <form action="process_add_employee.php" method="POST" enctype="multipart/form-data" id="projectAddForm" autocomplete="off">
                    <div class="form-section" style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
                        ...
                        <div class="form-group">
                            <label>&nbsp;</label>
                            <input type="text" id="proyek_input" name="proyek_otomatis" readonly style="background-color:#e9ecef;cursor:not-allowed;" />
                            <small id="proyek_tag" class="muted" style="display:block;visibility:hidden;">Project: —</small>
                        </div>
                    </div>
                    <input type="hidden" name="__labels" id="__labels_map" />
                    <div id="dynamic_fields" class="form-section" style="display:none;grid-template-columns:1fr 1fr;gap:12px"></div>
                    <div class="form-buttons" id="submit_container" style="display:none;margin-top:10px;">
                        <button type="submit" class="submit-btn"><i class="fas fa-save"></i> Simpan Karyawan</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                <div class="modal-body">
                    <form action="process_edit_employee.php" method="POST" id="editForm" autocomplete="off">
                        ...
                        <div class="form-section" style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
                            <div class="form-group">
                                <label>Project</label>
                                <input type="text" id="edit_proyek_text" readonly style="background:#eef2f7;cursor:not-allowed">
                                <input type="hidden" name="proyek" id="edit_proyek">
                            </div>
                            <div class="form-group"><label>&nbsp;</label><small class="muted">Field akan menyesuaikan project di kiri.</small></div>
                        </div>
                        <div id="edit_dynamic_fields" class="form-section" style="display:none;grid-template-columns:1fr 1fr;gap:12px"></div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.openViewModal = function (id_karyawan) {
        ...
        xhr.onload=function(){
            if(this.status==200){
                ...
                if(!data||data.error){ if(gridPribadi)gridPribadi.innerHTML="<p>Data tidak ditemukan.</p>"; }
                else{
                    ...
                    const pekerjaan=[
                        ["Project",data.proyek],["Jabatan",data.jabatan],
                ...
                viewModal.style.display="block"; bindTabs();
            } else {
    ...
    function inputFor(label){
        ...
        if (slug === "rt_rw") {
            return `<label>${label}
                <input type="text" name="${name}" placeholder="000/000" inputmode="numeric" pattern="\\d{3}/\\d{3}" ${req}>
            </label><small class="muted">Format wajib 3 angka, “/”, 3 angka. Contoh: 005/012</small>`;
        }
        const isNumeric = slug.startsWith("no_") || slug.includes("nomor") || slug.includes("nik") ||
                          ...
                          slug.includes("sales_code") || slug.includes("reff") || slug.includes("no_kontrak") || slug.includes("nomor_kontrak") || slug === "no_hp";
        if (isNumeric) {
            if (slug.includes('nik')) {
                return `<label>${label}<input type="text" name="${name}" placeholder="${label}" inputmode="numeric" pattern="\\d{16}" minlength="16" maxlength="16" ${req}></label><small class="muted">Masukkan tepat 16 digit angka.</small>`;
            }
            return `<label>${label}<input type="text" inputmode="numeric" pattern="\\d*" name="${name}" placeholder="${label}" ${req}></label>`;
        }
    ...
    function renderFormForProject(proj){
        ...
        tag.textContent = `Project: ${proj}`; tag.style.visibility='visible';

        const map = {};
        let fields = PROJECT_FIELDS[proj].slice();
        ...
        // Tampilkan/sembunyikan End Date sesuai status_karyawan
    ...
    window.openEditModal = function (id_karyawan, proyekHint){
        ...
        if(hint && PROJECT_FIELDS[hint]){ editProyekText.value=hint; editProyek.value=hint; renderEditFormForProject(hint); }
        else { editProyekText.value='-'; editProyek.value=''; editBox.innerHTML='<p>Pilih project…</p>'; editBox.style.display='block'; }
        ...
        xhr.onload=function(){
            if(this.status==200){
                ...
                if(proj && PROJECT_FIELDS[proj]){
                    if(editProyek.value!==proj){ editProyekText.value=proj; editProyek.value=proj; renderEditFormForProject(proj); }
                    fillFormValues(editBox,data);
                    // setelah terisi, sesuaikan end date
                    applyStatusEndDateLogic(editBox);
                }else{
                    editBox.innerHTML='<p>Project tidak dikenali.</p>';
                }
            }else{ editBox.innerHTML='<p>Error mengambil data.</p>'; }
    ...
    (function(){
        function getIdFrom(el){
            if(!el) return null;
            const dataId=el.getAttribute('data-id'); if(dataId) return dataId;
            const onclick=el.getAttribute('onclick')||''; const m=onclick.match(/'([^']+)'/); return m?m[1]:null;
        }
        document.addEventListener('click', function(e){
            ...
            if(editBtn){ e.preventDefault(); e.stopPropagation(); const id=getIdFrom(editBtn); const pj=(editBtn.getAttribute('data-proyek')||'').toUpperCase(); if(id) openEditModal(id,pj); return; }
...
(function(){
    ...
    if(OLD && OLD.proyek && sel){ sel.value=OLD.proyek; if(typeof renderFormForProject==='function') renderFormForProject(sel.value); }
    setTimeout(()=>{
        ...
        Object.keys(OLD).forEach(name=>{
            ...
            if(el.type==='date'){
                ...
                else { const d=new Date(v); if(!isNaN(d)){ const iso=new Date(d.getTime()-d.getTimezoneOffset()*60000).toISOString().slice(0,10); el.value=iso; } }
